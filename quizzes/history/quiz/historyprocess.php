...
public function get_mcq_ids()
{
    try
    {
        ...
        $selectQuery = "SELECT `history_quiz_id` FROM `HistoryQuiz`;";
        ...
        {
            $mcq_ids[] = $mcq_id_array["history_quiz_id"];            
        }
        return $mcq_ids;
    }
...
public function get_mcqs($mcqids)
{
    try
    {
        ...
        $selectQuery = "SELECT * FROM `HistoryQuiz` WHERE `history_quiz_id` IN ({$mcqids});";
