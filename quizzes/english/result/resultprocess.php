...
public function add_result($english_result_marks, $student_fullname)
{
    try
    {
        $reslt["english_result_marks"] = $english_result_marks;
...
public function get_results_by_user($student_fullname)
{
    try
    {
        ...
        $selectQuery = "SELECT * FROM EnglishResults JOIN students USING(student_fullname) JOIN classes USING(class_id) WHERE staff_email='" . $email . $googleEmail . "';";
