...
if (isset($_POST['delete'])) {
  ...
  // DELETE Query
  $query = "DELETE FROM EnglishResults WHERE english_result_id = {$delete_id}";
  ...
  if (mysqli_query($conn, $query)) {
    ...
    header('refresh:1; url=englishresults.php');
  }
  else {
    // Failed
    // Returns error
...
<head>
	<!-- Basic Page Needs -->
	<title>English Results | CloseApart</title>
	<meta charset="utf-8">
					...
					<div class="card">
						<div class="card-header bg-transparent">
							<div class="row align-items-center">
								<div class="col">
									<h3 class="mb-0">English Quiz Results</h3>
								</div>
							</div>
						</div>
						<div class="card-body">
								...
								<table id="sorttable" class="table align-items-center table-flush hover stripe"
									...
									?>
										<tr>
											...
											<td>
												<?php echo $result["english_result_date"]; ?>
											</td>
											<td>
												<?php echo $result["english_result_marks"]*10; ?>%
											</td>
											<td>
												<div class="d-flex">
													<form class="d-flex" method="POST" action="<?php $_SERVER['PHP_SELF']; ?>">
														<input type="hidden" name="delete-id" value="<?php echo $result['english_result_id']; ?>">
